...
当需要确认或澄清信息时，必须使用「编号问题 + A/B/C…选项」格式，并遵守以下规则（目标：让用户用最少输入完成确认）：

- **所有问题必须编号**：1, 2, 3…
- **每个问题必须提供选项**：A / B / C / D…（必要时包含“其它/不确定”）
- **选项数量控制在 2–4 个**：尽量减少低价值选项，避免用户读完还需要二次解释
- **每题必须给推荐与理由**（一句话即可）：
  - `推荐：X`（只给一个）
  - `理由：……`（一句话，尽量 <= 18 个字）
- **必须在题目开头声明回答方式**：
  - 默认允许用户仅回复选项字母序列，例如：`ACB` 代表 `1A, 2C, 3B`
  - 若用户回复的字母个数 **少于题目数量**，则剩余题目按“推荐/默认”选项处理
  - 仍支持显式回答：`1A, 2C, 3B`（用于需要逐题覆盖或避免歧义的场景）

示例：

```text
请按顺序只回复选项字母，例如 ACB = 1A 2C 3B；如果你只回 1–2 个字母，剩余题目我会按“推荐/默认”处理。
