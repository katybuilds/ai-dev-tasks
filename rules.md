...
- 只有当用户明确发出「log」相关指令（例如：“写 log” 或 “帮我记录到 dev-log”）时，AI 才会对 `docs/dev-log.md` 进行修改；
- 在实际写入日志之前，AI 应先根据本次改动，输出一个简短的大纲预案：
  - 使用项目符号列出若干要点，每个要点控制在约 5–10 个字，用来概括「改了什么 / 为什么改 / 下一步」等核心信息；
  - 等待用户确认或补充后，再据此大纲撰写完整的日志条目并追加到 `docs/dev-log.md` 中；
- 若用户只希望有一个「变更摘要」而暂不更新 dev-log，AI 应按摘要形式回复，而不要修改文件。*** End Patch```">
