...
2. **提出澄清问题：** 在写 PRD 前，最多只问最关键的 3 个问题（默认 2 个也可），用于补齐影响 PRD 清晰度的缺口。重点理解“做什么/为什么”，而不是“怎么做”。问题必须以编号 + 选项（A/B/C…）形式给出，并允许用户用最少输入回复（见下文“提问格式要求”）。
3. **等待用户回答并确认默认值：** 若用户回复字母数量少于题目数量，剩余题目按“推荐”选项处理（除非用户明确说不想默认）。若用户回复“你决定/按推荐/默认就好”，则直接按推荐值继续，不再追问。
4. **生成 PRD：** 基于初始需求与用户回答，按下述结构生成 PRD。
5. **保存 PRD：** 将文件保存为 `/docs/PRD.md`。

## 小白防呆模式（默认启用）

当用户没有提供技术细节时，智能助手必须自动采用以下默认策略，不要求用户做技术判断：

- 默认采用 **移动端优先（mobile-first）**。
- 默认覆盖断点：`320 / 375 / 390 / 412 / 768 / 1024`。
- 默认要求：页面级 `no horizontal overflow`（不允许全页横向滚动）。
- 默认要求：宽内容（表格、图表、代码块）采用组件内横向滚动，而不是撑宽页面。
- 默认要求：表单控件遵循可收缩规则（中间输入可缩，按钮与单位不被挤出）。
- 默认输出移动端验收清单，且写成“可验证语句”（不是抽象建议）。

只有当用户明确要求“仅桌面端”时，才允许放宽上述默认策略。
...
- **移动端优先级与断点：** 若缺失，优先直接采用默认值（见“小白防呆模式”），通常不单独提问。
- **移动端交互策略：** 若缺失，优先直接写入默认策略（导航横滑/宽内容组件内滚动/防溢出约束），通常不单独提问。
- **移动端验收标准：** 若缺失，必须自动写入默认验收项，不能留空。
...
8. **Mobile & Responsive Requirements（移动端与响应式要求，必写）：**
   - 断点范围（至少包含 320/375/390/412，按项目可扩展）；
   - 移动端布局策略（堆叠/横滑/组件内滚动）；
   - 防溢出规则（如 `min-w-0 / w-full / max-w-full` 原则）；
   - 移动端验收项（不横向溢出、关键按钮可见、核心流程可完成）。
9. **Non-Goals (Out of Scope)：** 明确不做的范围。
10. **Design Considerations (Optional)：** 设计/组件/样式指引。
11. **UI Reference Code（UI 参考代码，必留占位）**：
...
12. **Technical Considerations (Optional)：** 技术约束或依赖。
13. **Success Metrics：** 成功衡量指标。
14. **Open Questions：** 未决问题。

## 移动端章节最小模板（必须落地到 PRD）

当生成 `Mobile & Responsive Requirements` 时，至少包含以下 4 组内容（可扩展，但不得缺项）：

1. **断点与设备范围**
   - 至少写明：`320 / 375 / 390 / 412`。
2. **布局与交互策略**
   - 导航：窄屏横滑或等价策略；
   - 宽内容：组件内滚动；
   - 表单：可收缩布局（输入可缩、按钮不丢失）。
3. **防溢出约束**
   - 明确写出容器约束原则（如 `min-w-0 / w-full / max-w-full`）。
4. **可验证验收项**
   - 至少 5 条，且必须可直接测试（例如：`320px 下右侧 + 按钮完整可见`）。
...
5. `docs/PRD.md` 必须包含“Mobile & Responsive Requirements”章节，不得省略
6. PRD 必须写明移动端验收断点与至少 3 条可验证标准（如：无横向溢出、关键控件不裁切、320 宽可完成主流程）
7. 若用户未提供移动端细节，必须自动应用“小白防呆模式”默认值，不能把关键移动端策略留给用户决策
8. 除非用户明确要求“仅桌面端”，否则不得将移动端策略写为 `Open Questions`
