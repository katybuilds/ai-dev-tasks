...
3. **提出澄清问题（如必要）：** 若存在会明显影响任务拆分/范围/依赖的关键不确定因素，必须先提问澄清（通常 1–4 个），再进入任务生成阶段。提问格式必须满足下文要求，方便用户用最少输入回复。
4. **阶段 1：生成父任务：** 基于需求分析，生成高层任务（不含子任务）。父任务数量通常 4–6 个。将父任务按指定格式发给用户，并提示：“我已生成高层任务，是否继续生成子任务？回复 ‘Go’ 继续。”
5. **等待确认：** 暂停并等待用户回复 “Go”。
6. **阶段 2：生成子任务：** 用户确认后，为每个父任务拆分可执行子任务，确保覆盖实现细节。
7. **识别相关文件：** 列出可能需要创建或修改的文件，必要时包含测试文件。
8. **生成最终输出：** 合并父任务、子任务、相关文件与说明，形成最终文档。
9. **保存任务清单：** 写入 `/docs/tasks.md`。

## 澄清问题格式（必需）

当需要确认或澄清信息时，使用「编号问题 + A/B/C…选项」格式，并遵守：

- 问题必须编号（1, 2, 3…）
- 每题给出 2–4 个选项（A/B/C/D…）
- 每题必须给推荐与理由：
  - `推荐：X`
  - `理由：一句话`
- 必须在问题开头声明回答方式：
  - 允许用户按顺序只回字母，例如：`ACB` = `1A, 2C, 3B`
  - 若用户回的字母数量少于题目数量，剩余题目按“推荐”选项处理
