...
### 4.6 弹出层（Popup）与设置页（Options Page）
...
## 7. Technical Considerations（技术实现考虑）
...
- 本项目内部规格文档（如 `spec.md` / `style.md` / `tasks.md`）可以使用中文撰写，方便团队沟通与维护；
- 所有代码（变量名、函数名、注释）、对外界面文案以及部署相关配置统一使用英文；
- 更详细的语言使用范围和约束，请参考仓库根目录下 `rules.md` 中的《语言使用规范》一节。

**本地完工提示（可选）**

- 当本 PRD 对应的全部开发任务在本地完成后，如需声音提醒，可参见 `rules.md` 中的「本地完工提示」命令，在终端手动运行。*** End Patch`்கிற ***!
